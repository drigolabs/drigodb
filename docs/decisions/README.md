...
[0008](0008-mechanism-in-the-core-policy-outside.md) names the split these records
fall into. drigodb's **core** provides mechanisms — make a database exist, put it
to sleep, wake it, resize it, back it up. A **policy** layer decides *when* to use
them, and lives outside the core rather than inside it. A third group is neither:
how drigodb itself is run.

Grouped that way rather than by number, because the number says when a decision
was made and the group says what it governs.

### The core — what a database is, and what drigodb will do to one
...
### The policy layer — deciding when
...
Only one so far, and it is `proposed` rather than decided. That is the honest
state: the core is nearly complete and the layer above it has been designed
twice and built never. [0008](0008-mechanism-in-the-core-policy-outside.md) is
the argument for finishing the first before starting the second.

Policy components call the API. They are not directories in this repository —
a thing that decides when to hibernate a database talks to drigodb the way any
other consumer does, and the day one exists it will be its own deployable.
